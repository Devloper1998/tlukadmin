...
$crud = new Crud();
$tableName = 'tluk_eventcategories';
...
if(isset($_POST["action"]) && $_POST['action'] == 'Display'){
    $sql_show = "SELECT * FROM tluk_eventcategories order by id desc";
...
if(isset($_POST["action"]) && $_POST['action'] == 'Displays'){
    $sql_show = "SELECT * FROM tluk_eventcategories where id ='".$_POST['id']."' order by id desc ";
    $show_data = $crud->getData($sql_show);        
       $response = array(
        "draw" => 1,
        "recordsTotal" => count($show_data),
